	...
	if(EthInfo.socketportusedflag[sn] == 1){  //连接处理		
		if(EthInfo.LinkFlag == 1 ){	
			if(EthInfo.ConnCmd[sn] == 1){
				...
				netconn_bind(tcp_clientconn, IP_ADDR_ANY, 8000);//
				
				IP4_ADDR(&server_ipaddr, EthInfo.serverip[sn][0], EthInfo.serverip[sn][1], EthInfo.serverip[sn][2], EthInfo.serverip[sn][3]);
