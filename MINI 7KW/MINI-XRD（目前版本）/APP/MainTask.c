...
** 函 数 名 : JudgePeriod
** 功能描述 : 判断时间函数
** 输    入 : port 充电接口 =0代表1号枪 =1代表2号枪......    time 0-47 保留3位小数，精确到0.1分每度电
** 输    出 : 无
** 返    回 :	无
*************************************************************************************************************
*/
...
static void UpdateChargeData(MAININFO* ChgData, CHARGEINFO info, uint8_t port)
{
	...
	ChgData->ChgDat.SumMoney = (data32 + 900)/ 1000;//始终加0.009元，补后台的服务费和充电费分开的问题
	RtcRead(&datetime);
	...
	switch(OneGunState){
		case A_IDLE://空闲---------------------------------------------------------------------------------------------------------------------------------
			...
			if(( Updata>> 8) == 1){   //升级中
				...
				DispAIdleInfo(QRCode, PileNumber, info.GunSeatState);	
					//判断并处理刷离线卡操作
		...
		case A_CHARGE://充电---------------------------------------------------------------------------------------------------------------------------------
			...
			//更新显示屏数据
			DispAChargeInfo(MainInfo.ChgDat.SumEnergy, MainInfo.ChgDat.SumMoney, MainInfo.GunInfo.CurrentA, 0, 0,\
																																					 MainInfo.GunInfo.VoltageA,  0,  0,  MainInfo.ChgDat.SumTime);
			...
			if (SwingOfflineCardFlag == 1){//刷离线卡
				...
				if ((MainInfo.ChgDat.StartType == STARTCHARGETYPE_OFFLINECARD) && (CardID == MainInfo.ChgDat.StartCardID)){//刷卡停止
					...
					if(UnLockCardPro(CardID, money)){
						...
						OneGunState = A_IDLE;
						//显示结算界面
						DispAccount(MainInfo.ChgDat.SumEnergy, MainInfo.ChgDat.SumMoney, money, MainInfo.ChgDat.SumTime, MainInfo.ChgDat.StopDateTime);
		...
		case A_STOP://停止---------------------------------------------------------------------------------------------------------------------------------
			...
			//更新显示屏数据
			if( MainInfo.ChgDat.StartType ==  STARTCHARGETYPE_PLATFORM)
				DispAStopInfo((uint8_t *)"  ", (uint8_t *)stopcausetab[MainInfo.ChgDat.StopCause], MainInfo.ChgDat.SumEnergy, MainInfo.ChgDat.SumMoney);		
			else
				DispAStopInfo((uint8_t *)"请刷卡结算", (uint8_t *)stopcausetab[MainInfo.ChgDat.StopCause], MainInfo.ChgDat.SumEnergy, MainInfo.ChgDat.SumMoney);		
