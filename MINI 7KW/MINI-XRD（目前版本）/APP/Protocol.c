...
static COMMINFO CommInfo[1];
static REportCMD204  ReportCMD204[1];
...
static void SendCMD102(uint8_t port)
{
	...
	*pbuf++ = addChecksum(buffer + 6, pbuf - buffer - 6);
	...
	printf("CMD102 发送充电桩上传心跳包信息P%d:", port);
	printfx(buffer, pbuf - buffer);
...
static void RecvCMD1(uint8_t port, uint8_t *buffer)
{
	...
	if(type == 0x00){ //查询
		...
		*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
		...
		printf("CMD2 充电桩参数整形查询应答P%d:", port);
		printfx(ackbuffer, pbuf - ackbuffer);
	}
	if(type == 0x01){ //设置
		ret = 0;
		paranum = buffer[17];
		...
		*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
		...
		NetSend(port, ackbuffer, pbuf - ackbuffer);
		printf("CMD2 充电桩参数整形设置应答P%d:", port);
		printfx(ackbuffer, pbuf - ackbuffer);
	}
}
...
static void RecvCMD3(uint8_t port, uint8_t *buffer)
{
	...
	if(type == 0x00){ //查询
		...
		*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
		...
		NetSend(port, ackbuffer, pbuf - ackbuffer);
		printf("CMD4 充电桩参数字符形查询应答P%d:", port);
		printfx(ackbuffer, pbuf - ackbuffer);
	}
	if(type == 0x01){ //设置
		switch(paraaddr){
		...
		*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
		...
		NetSend(port, ackbuffer, pbuf - ackbuffer);
		printf("CMD4 充电桩参数字符形设置应答P%d:", port);
		printfx(ackbuffer, pbuf - ackbuffer);
	}
}
...
//	cmdlen = buffer[18] | (buffer[19] << 8);
	...
	*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
	...
	NetSend(port, ackbuffer, pbuf - ackbuffer);
	printf("CMD6 充电桩对后台控制命令应答P%d:", port);
	printfx(ackbuffer, pbuf - ackbuffer);
...
static void SendCMD8(uint8_t port, uint32_t failcode)
{
	...
	*pbuf++ = addChecksum(buffer + 6, pbuf - buffer - 6);
	...
	NetSend(port, buffer, pbuf - buffer);
	printf("CMD8 充电桩对后台下发的充电桩开启充电控制应答P%d:", port);
	printfx(buffer, pbuf - buffer);
}
...
//	*pbuf++ = 0x00;
	...
	*pbuf++ = 0x02; //充电枪类型 01直流 02交流
	...
	if (MainState == 0){
		if ((info.ScramState == 1)|| GetLifeTime()|| (ElmGetCommState() == 1) ){
			*pbuf++ = 6; //工作状态 故障
...
//	pbuf += 2;
	...
	*pbuf++ = 0x00;//info.Mode; //BMS充电模式
	...
	memcpy(pbuf, &data32, 2);
	pbuf += 2;
	data32 = GetSumTime(port);//0; //充电时长 
	memcpy(pbuf, &data32, 4);
	pbuf += 4;
	data32 = GetSumEnergy(port);//0; //本次充电累计充电电量
	memcpy(pbuf, &data32, 4);
	pbuf += 4;
	data32 = GetStartMeterEnergy(port);//0; //充电前电表读数 
	memcpy(pbuf, &data32, 4);
	pbuf += 4;
	data32 = GetStopMeterEnergy(port);//0; //当前电表读数 
	memcpy(pbuf, &data32, 4);
	pbuf += 4;
	if (GetStartType(port) == 1)
		...
		*pbuf++ = 0x00; //充电启动方式  在线卡和离线卡
	*pbuf++ = GetStartMode(port); //充电策略  
	data32 = GetStartPara(port);//0; //充电策略参数  
	memcpy(pbuf, &data32, 4);
	pbuf += 4;
	*pbuf++ = 0x00; //预约标志 需改----------------
...
//	memcpy(pbuf,CommInfo[port].ChargeCardNumber,32);  
	...
	*pbuf++ = 0xff;
	...
	pbuf += 12;
	*pbuf++ = addChecksum(buffer + 6, pbuf - buffer - 6);
	printf("CMD104 充电桩状态信息包上报P%d:", port);
	printfx(buffer, pbuf - buffer);	
	NetSend(port, buffer, pbuf - buffer);
...
*************************************************************************************************************
*/
static void SendCMD202(uint8_t port, CHARGINGDATA *ChgDat, uint8_t current_index)
{
	...
	*pbuf++ = 0;
	...
	if(ChgDat->StartType == STARTCHARGETYPE_PLATFORM){  // 修改 APP 充电后用离线卡会上报 UserId
		UseId = ChgDat->StartCardID;
		memcpy(pbuf,&UseId, 2);
	}else{
		UseId = 0;
		memcpy(pbuf,&UseId, 2);
	...
	*pbuf++ = 0;
	memset(pbuf, 0, 4); 
	memcpy(pbuf, &ChgDat->SumEnergy, 2); //本次充电电量
	pbuf += 4;
	memset(pbuf, 0, 4); 
	memcpy(pbuf, &ChgDat->StartMeterEnergy, 2); //充电前电表读数
	pbuf += 4;
	memset(pbuf, 0, 4); 
	memcpy(pbuf, &ChgDat->StopMeterEnergy, 2); //充电后电表读数
	pbuf += 4;
	memset(pbuf, 0, 4); 
	memcpy(pbuf, &ChgDat->SumMoney, 2); //本次充电金额
	pbuf += 4;
	memset(pbuf, 0, 4); //预留
	pbuf += 4;
	...
	pbuf += 4;
	memset(pbuf, 0, 4);
	memcpy(pbuf, &current_index, 1);//当前充电记录索引//memcpy(pbuf, &CommInfo[port].ChargeRecordIndexBak, 4);
	pbuf += 4;
	memcpy(pbuf, &CommInfo[port].ChargeRecordNumberBak, 4);//总充电记录条目
	pbuf += 4;
	*pbuf++ = 0; //预留
	*pbuf++ = ChgDat->Mode; //充电策略
	memset(pbuf, 0, 4);//memcpy(pbuf, &ChgDat->Para, 4); //充电策略参数
	pbuf += 4;
...
//	pbuf += 284;
	*pbuf++ = addChecksum(buffer + 6, pbuf - buffer - 6);
	...
	NetSend(port, buffer, pbuf - buffer);
	printf("CMD202 充电桩上报充电记录信息P%d:", port);
	printfx(buffer, pbuf - buffer);
}
...
//	buf1[10]=0x30;
	...
	*pbuf++ = addChecksum(buffer + 6, pbuf - buffer - 6);
	...
	NetSend(port, buffer, pbuf - buffer);
	printf("CMD204 充电桩充电上传用户帐户查询报文P%d:", port);
	printfx(buffer, pbuf - buffer);
}
...
static void SendCMD106(uint8_t port)
{
  ...
  memcpy(pbuf, PileNumber, 32);
	...
	*pbuf++ = 0;
	*pbuf++ = addChecksum(buffer + 6, pbuf - buffer - 6);
	printf("CMD106 发送充电桩签到信息P%d:", port);
	printfx(buffer, pbuf - buffer);	
	NetSend(port, buffer, pbuf - buffer);
...
static void RecvCMD1011(uint8_t port, uint8_t *buffer)
{
	...
	*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
	...
	NetSend(port, ackbuffer, pbuf - ackbuffer);
	printf("CMD1012 充电桩应答服务器下发重启命令P%d:", port);
	printfx(ackbuffer, pbuf - ackbuffer);
	printf("准备重启设备..................\r\n\n\n");
	OSTimeDlyHMSM (0, 0, 3, 0);
	//是否需要保存数据再重启
...
static void RecvCMD1101(uint8_t port, uint8_t *buffer)
{
	...
	*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
	...
	NetSend(port, ackbuffer, pbuf - ackbuffer);
	printf("CMD1102 充电桩应答后台服务器查询24小计价策略信息P%d:", port);
	printfx(ackbuffer, pbuf - ackbuffer);
}
...
static void RecvCMD1103(uint8_t port, uint8_t *buffer)
{
	...
	*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
	...
	NetSend(port, ackbuffer, pbuf - ackbuffer);
	printf("CMD1104 充电桩应答后台服务器设置24小计价策略信息P%d:", port);
	printfx(ackbuffer, pbuf - ackbuffer);
}
...
static void RecvCMD1001(uint8_t port, uint8_t *buffer)
{
	...
	if((buffer[8]== 0x55) && (buffer[9] == 0xaa)){
		if(GetMainCharge(0) != STATE_CHARGE ){	
			if(procress_erase()){
				ret = 0;
				printf("擦除存储程序区数据错误P%d:\r\n", port);
			}else{
				ret = 100;
				printf("擦除存储程序区数据正确P%d:\r\n", port);
				UpdataFlag = 0x01;
	...
	*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
	...
	NetSend(port, ackbuffer, pbuf - ackbuffer);
	printf("CMD1002 充电桩应答后台服务器擦除命令P%d:", port);
	printfx(ackbuffer, pbuf - ackbuffer);
}
...
//	memcpy(FileName, &buffer[8], datalen - 9); 
	printf("CMD1004 充电桩应答后台服务器下发升级文件名P%d \r\n", port);
	UpdataTimer = TimerRead();
	pbuf = ackbuffer;
	*pbuf++ = STARTCODE1; //起始域
	*pbuf++ = STARTCODE2;
	...
	*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
	...
	NetSend(port, ackbuffer, pbuf - ackbuffer);
	printf("CMD1004 充电桩应答后台服务器下发升级文件名指令P%d:", port);
	printfx(ackbuffer, pbuf - ackbuffer);
}
/************************************************************************************************************
** 函 数 名 : RecvCMD1005
...
static void RecvCMD1005(uint8_t port, uint8_t *buffer)
{
  ...
  UpdataFileSize = buffer[8] | (buffer[9]<<8) | (buffer[10]<<16) |(buffer[11]<< 24);
	if( UpdataFileSize > MAXAPPSIZE){  //升级文件大于程序存储区
		data32 = 1;
		UpdataFileSize = 0;
	}else{
		data32 = 0;
	...
	*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
	NetSend(port, ackbuffer, pbuf - ackbuffer);
	printf("CMD1006 充电桩应答后台服务器下发升级文件大小指令P%d:", port);
	printfx(ackbuffer, pbuf - ackbuffer);
	FileIndexNum = 0;
}
...
static void RecvCMD1007(uint8_t port, uint8_t *buffer)
{
	...
	printf("CMD1007 数据序列：%d  FileIndexNum = %d 数据长度= %d\r\n", IndexNum, FileIndexNum, datalen -9 );
	UpdataTimer = TimerRead();
	buffer = buffer + 8;
	if(IndexNum == FileIndexNum){
		SN = 0x00;
		process_update(IndexNum, buffer,datalen -9);//写数据到 程序存储区
		FileIndexNum++;
	}else{
		ret = 0x01;
		SN = 0x01;
	...
	*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
	if(	UpdataFlag ){   
		printf("CMD1008 充电桩应答后台服务器下发升级数据指令P%d:", port);
		printfx(ackbuffer, pbuf - ackbuffer);
		NetSend(port, ackbuffer, pbuf - ackbuffer);
	}
...
static void RecvCMD1009(uint8_t port, uint8_t *buffer)
{
	...
	*pbuf++ = addChecksum(ackbuffer + 6, pbuf - ackbuffer - 6);
	if(UpdataFlag ){
		NetSend(port, ackbuffer, pbuf - ackbuffer);
		printf("CMD1010 充电桩应答服务器下发升级文件数据结束指令P%d:", port);
		printfx(ackbuffer, pbuf - ackbuffer);
		SetApplenth(UPDATAFLAG , UpdataFileSize);
	}
...
static void SendCMD702(uint8_t port, uint8_t Result)
{
	...
	*pbuf++ = addChecksum(buffer + 6, pbuf - buffer - 6);
	NetSend(port, buffer, pbuf - buffer);
	printf("CMD702 充电桩应答修改充电中余额%d:", port );
	printfx(buffer, pbuf - buffer);
}
...
static void AccountQueryReport(uint8_t port)
{
	...
	static uint32_t report204timer[1];
...
static void ReportChargeRecord(uint8_t port)
{
	...
	static uint32_t IsStopReport[1];
	CHARGINGDATA ChgDat;
	uint16_t index,i;
		...
		if ((CommInfo[port].ChargeRecordNumberBak != GetChargeRecordNumber(port)) || (CommInfo[port].ChargeRecordIndexBak != GetChargeRecordIndex(port))){//新交易记录
			...
			if(GetChargeRecord(port, &ChgDat)){
				...
				for (i = 0; i < REPORT_NUM - 1; i++)
					...
					CommInfo[port].NeedReportNum++;	
			}else if (FindChargeRecord(index,&ChgDat) == 1){//查询成功
				for (i = 0; i < REPORT_NUM - 1; i++)
					CommInfo[port].ReportCtl[REPORT_NUM - 1 - i] = CommInfo[port].ReportCtl[REPORT_NUM - 2 - i];
					CommInfo[port].ReportCtl[0].index = index;
					CommInfo[port].NeedReportNum++;
			...
			if(CommInfo[port].NeedReportNum > 0){
				if((CommInfo[port].ReportCtl[0].index == index)||(CommInfo[port].ReportCtl[0].index == index-1) || ((index == 0)&& (CommInfo[port].ReportCtl[0].index == RECORD_TOTAL_NUM-1))) {
					if(GetChargeRecord(port, &ChgDat)){
						...
						FindChargeRecord(CommInfo[port].ReportCtl[0].index,&ChgDat);
					}
				}else{
					FindChargeRecord(CommInfo[port].ReportCtl[0].index, &ChgDat);
				}
...
static void ProtocolHandle(uint8_t port)
{
	...
	static uint8_t logincount[1], heartbeatcount[1];
	static uint32_t timer[1], reportstatetimer[1];
	static uint32_t reportstartchargeresulttimer[1];
	static uint8_t chgstate[1];
	uint8_t sip[4];
...
static void ProtocolInit(void)
{
	...
	CommInfo[0].ChargeRecordIndexBak = index;
	num = GetChargeRecordNumber(0) % REPORT_NUM;
	if((GetChargeRecordNumber(0) / REPORT_NUM) != 0)
		num = REPORT_NUM;
	CommInfo[0].ChargeRecordNumberBak = num;
	for (i = 0; i < num; i++){
		if (FindChargeRecord(index, &ChgDat) == 1)//查询成功
			CommInfo[0].ReportCtl[CommInfo[0].NeedReportNum++].index = index;
		if (index == 0)
