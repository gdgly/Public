...
#include "lcd_12864.h"
#include "Para.h"
static uint8_t Qrcode[246];  //存储12864显示二维码图片字模
...
static unsigned char m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE];
...
/************************************************************************************************************
** 函 数 名 : ChangeTo12864Data
** 功能描述 : 输出12864显示屏 取模数组
** 输    入 : 无
** 输    出 : 无
** 返    回 : 无
*************************************************************************************************************
*/
void ChangeTo12864Data(void)
{
	uint8_t i,j;
	memset(Qrcode, 0, sizeof(246));
	printf("Qrsize:%d*%d\r\n",m_nSymbleSize,m_nSymbleSize);
	for(i=0; i< m_nSymbleSize; i++){
		for(j=0;j< m_nSymbleSize;j++){
			if(m_byModuleData[i][j] == 1){
				if((i%8)==0){
					Qrcode[j+ ((i/8)*m_nSymbleSize)] |=(1<<0);
				}else if((i%8)==1){
					Qrcode[j + ((i/8)*m_nSymbleSize)] |=(1<<1);
				}else if((i%8)==2){
					Qrcode[j + ((i/8)*m_nSymbleSize)] |=(1<<2);
				}else if((i%8)==3){
					Qrcode[j + ((i/8)*m_nSymbleSize)] |=(1<<3);
				}else if((i%8)==4){
					Qrcode[j + ((i/8)*m_nSymbleSize)] |=(1<<4);
				}else if((i%8)==5){
					Qrcode[j + ((i/8)*m_nSymbleSize)] |=(1<<5);
				}else if((i%8)==6){
					Qrcode[j + ((i/8)*m_nSymbleSize)] |=(1<<6);
				}else if((i%8)==7){
					Qrcode[j + ((i/8)*m_nSymbleSize)] |=(1<<7);
				}		
			}else{
				if((i%8)==0){
					Qrcode[j+ ((i/8)*m_nSymbleSize)] &=~(1<<0);
...
*************************************************************************************************************
*/
void InitQrCodeData(void)
{
	uint8_t PileNumber[32];
	ParaGetPileNo(0, PileNumber);
	sprintf((char *) Qrcode,"http://chargepile.chargepile.tomorn.cn/%sA", PileNumber);//最大长度54个字符
	EncodeData((char *)Qrcode);
	ChangeTo12864Data();
}
/************************************************************************************************************
** 函 数 名 : LcdShowQrCode
...
*************************************************************************************************************
*/
void LcdShowQrCode(uint8_t x, uint8_t y )
{
	LCD_DrawImage(x,y,m_nSymbleSize,32,Qrcode);
}
