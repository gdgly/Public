...
//fAPB2时钟一般为84Mhz：
static void SPI1_SetSpeed(u8 SPI_BaudRatePrescaler)
{
...
uint32_t GetApplenth(uint8_t flag)
{
	...
	FramRead(FRAMADDR_UPDATAINFO, (u8 *)&UpdataInfo, sizeof(UPDATAINFO));
	if(flag){
		if(UpdataInfo.FLAG == 0x5a5a){
		UpdataInfo.FLAG = 0x00;
		FramWrite(FRAMADDR_UPDATAINFO,(u8 *)&UpdataInfo, sizeof(UPDATAINFO));
